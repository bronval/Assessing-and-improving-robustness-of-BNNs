...
class SimpleMLP(SeqBinModelHelper, nn.Module, ModelHelper):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            InputQuantizer(0.1),
            Flatten(),
            BinLinearPos(w_binarizer, 32*32*3, 512, rounding=False),
...
# model = train_test_model(epochs=3, name="mlp_model_cifar.pth")
model = SimpleMLP()
model.load_state_dict(torch.load("mlp_model_cifar.pth"))
...
def show_picture(data, index, adv=False, img=None, eps=None, clsfy=None):
    ...
    if adv:
        ...
        file_name = "imgs_cifar10_dataset/cifar10_image_" + str(index) + "_adv_" + str(eps) + ".png"   # R
    else:
        plt.title(f"classified as : {clsfy}")
        file_name = "imgs_cifar10_dataset/cifar10_image_" + str(index) + ".png"
...
for idx in idxs:
    for inputs, labels in test_dataloader:
        ...
        model.eval()
        with torch.no_grad():
            pred = model(torch.transpose(inputs[idx].flatten(), -1, 0).reshape((1, 3072)))
